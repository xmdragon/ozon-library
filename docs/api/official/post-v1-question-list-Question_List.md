...
| `limit` | integer <int64> <= 100 响应中返回的值数量。 |
| `sort_dir` | string Default: "DESC" Enum: "DESC" "ASC" 排序方向： DESC——降序； ASC——升序。 |

### 表格 2

| 字段 | 类型/说明 |
| --- | --- |
| `questions` | Array of objects [ 0 .. 10 ] items 问题。 |
| `last_id` | string 页面上最后一个值的标识符。 要获取下一个批次的数据，请在下一个请求的 last_id 参数中传递上次获取的值。 |
| `has_next` | boolean 如果响应中未返回所有问题，则为true。 |
