...
count required integer <int32> 响应中的商品数量。 last_idinteger <int64> 页面上最后一个值的ID。运行第一个查询时，将此字段留空。 要检索以下数值，请从上一个查询的响应中指定last_id。

### 表格 1

has_nextboolean true，如果响应中没有返回所有值。 itemsArray of objects 草稿。 last_idinteger <int64> 页面上最后一个值的标识符。
