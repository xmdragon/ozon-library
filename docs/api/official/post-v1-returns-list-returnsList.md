...
filterobject 筛选条件。在请求中仅使用一个筛选器：logistic_return_date、storage_tariffication_start_date 或visual_status_change_moment，否则会返回错误。 limit required integer <int32> <= 500 加载的退货数量。 last_idinteger <int64> 最后加载的退货ID。

### 表格 1

returnsArray of objects 退货信息。 has_nextboolean 如果卖家有其他退货，显示true。
