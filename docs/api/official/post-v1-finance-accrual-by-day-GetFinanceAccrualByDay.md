...
date required string YYYY-MM-DD 应计日期。最早可查询日期为2022年1月1日。 last_id required string 页面上最后一个值的标识符。首次请求请留空。 要获取后续值，请指定上一次请求响应中的 last_id。

### 表格 2

accrualsArray of objects 应计项目列表。 last_idstring 页面中最后一个值的标识符。
