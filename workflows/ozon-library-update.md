...
2. 确认 Chrome 打开 Ozon Seller API 官方文档：`https://docs.ozon.ru/api/seller/zh/?__rr=1`，并可访问 `https://docs.ozon.ru/api/seller/zh/#tag/News`。
3. 按 `workflows/chrome-official-seller-api-extract.md` 通过 Chrome DOM 更新 `indexes/official-seller-api.operations.json` 和 `indexes/official-seller-api.news.json`，运行 `python3 tools/apply_official_api_news.py` 合并 News 标记，再运行 `python3 tools/generate_official_api_docs.py` 更新 `docs/api/official/`。
...
python3 -m json.tool indexes/official-seller-api.news.json >/tmp/official-news.check
...
python3 -m unittest tests/test_generate_official_api_docs.py tests/test_apply_official_api_news.py -v
```
